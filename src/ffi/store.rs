...
pub extern "C" fn askar_store_copy(
    ...
    catch_err! {
        ...
        let pass_key = PassKey::from(pass_key.as_opt_str()).into_owned();
        let tenant_profile = tenant_profile.into_string();
        let cb = EnsureCallback::new(move |result|
            match result {
        ...
        spawn_ok(async move {
            let result = async move {
                let store = handle.load().await?;
                let copied = store.copy_to(target_uri.as_str(), key_method, pass_key.as_ref(), recreate != 0, tenant_profile.as_str()).await?;

    ...
    HmacKey: KeyGen + HmacOutput,
{
    ...
    pub fn encrypt(mut buffer: SecretBytes, enc_key: &Key) -> Result<SecretBytes, Error> {
    ...
    pub fn decrypt(mut buffer: SecretBytes, enc_key: &Key) -> Result<SecretBytes, Error> {
    ...
    pub fn encrypt_tag_name(&self, name: SecretBytes) -> Result<SecretBytes, Error> {
        Self::encrypt_searchable(name, &self.tag_name_key, &self.tags_hmac_key)
    }

    pub fn encrypt_tag_value(&self, value: SecretBytes) -> Result<SecretBytes, Error> {
        Self::encrypt_searchable(value, &self.tag_value_key, &self.tags_hmac_key)
    }

    pub fn decrypt_tag_name(&self, enc_tag_name: SecretBytes) -> Result<SecretBytes, Error> {
        Self::decrypt(enc_tag_name, &self.tag_name_key)
    }

    pub fn decrypt_tag_value(&self, enc_tag_value: SecretBytes) -> Result<SecretBytes, Error> {
    ...
    HmacKey: KeyGen + HmacOutput,
{
    ...
    fn decrypt_entry_value(&self, mut enc_value: SecretBytes) -> Result<SecretBytes, Error> {
        ...
        let value = SecretBytes::from_slice(&enc_value[enc_key_size..]);
    ...
    fn decrypt_entry_tags(&self, enc_tags: Vec<EncEntryTag>) -> Result<Vec<EntryTag>, Error> {
        enc_tags.into_iter().try_fold(vec![], |mut acc, tag| {
            ...
            acc.push(if tag.plaintext {
                let value = decode_utf8(tag.value.into_vec())?;
                EntryTag::Plaintext(name, value)
            } else {
...
mod tests {
    ...
    fn encrypt_entry_round_trip() {
        ...
        let enc_tags = key
            ...
            .unwrap();
        assert_ne!(test_record.category.as_bytes(), enc_category.as_ref());
        assert_ne!(test_record.name.as_bytes(), enc_name.as_ref());
